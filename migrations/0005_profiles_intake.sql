...
CREATE TABLE IF NOT EXISTS intake_answers (
  ...
  UNIQUE(account_id, line_user_id, intake_form_id, question_key)
);
...
-- インデックス
CREATE INDEX IF NOT EXISTS idx_user_profiles_account_line ON user_profiles(account_id, line_user_id);
CREATE INDEX IF NOT EXISTS idx_intake_forms_account_id ON intake_forms(account_id);
CREATE INDEX IF NOT EXISTS idx_intake_answers_account_line ON intake_answers(account_id, line_user_id);
...
INSERT OR IGNORE INTO question_definitions (question_key, category, label_ja, input_type, is_required, order_index) VALUES
  ...
  ('diet_history', 'intake', 'これまでのダイエット経験', 'text', 0, 10),
  ('medical_notes', 'intake', '持病・注意事項（任意）', 'text', 0, 11),
