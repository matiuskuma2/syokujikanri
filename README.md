...
> **💡 初期運用の推奨**: staffロールはコード上互換性のため残していますが、**初期運用ではsuperadmin + adminの2ロール構成を推奨**します。管理画面のadmin作成フォームもadminロール固定で簡素化されています。staffが必要になった場合はDB直接操作で追加可能です。
...
- **最終デプロイ**: 2026-03-12（Cloudflare Pages 本番デプロイ完了）
- **デプロイURL**: https://bc7caa3a.diet-bot.pages.dev
...
2. 実機テスト: superadmin→admin→userの完全フロー検証（上記チェックリスト参照）
