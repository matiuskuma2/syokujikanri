...
| G5 | Rich Menu 未設定 | 低 | モード切替がテキストコマンドのみ | Phase 2 で設定 |

### 10.2 改善優先順位

1. **【即時】** E2Eテスト（全フロー通し確認）
2. **【即時】** 本番マイグレーション・seed投入
3. **【短期】** CONSULT_KEYWORDS をモード自動切替に組み込み
4. **【短期】** Rich Menu 設定（記録/相談/ダッシュボード）
5. **【中期】** 画像解析 × food_master マッチング統合
6. **【中期】** RAG実装（LIKE検索→ベクトル検索）
7. **【中期】** 管理画面プロンプトエディタ
