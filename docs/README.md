...
| `docs/11_会話解釈SSOT.md` | 会話解釈アーキテクチャ（Phase A: Unified Intent Schema、日付/食事区分解決ルール、保存確認ルール、修正ターゲット特定）（**v2.0正本 v1.1**） |
| `docs/12_記録確認フローSSOT.md` | 記録確認フロー（Phase B: 明確化、Phase C: 永続化、修正フロー、保存判定フロー）（**v2.0正本 v1.1**） |
| `docs/13_パーソナルメモリSSOT.md` | パーソナルメモリ（user_memory_items、嗜好・習慣・アレルギー蓄積）（**v2.0正本**） |
| `docs/14_技術設計チェックリストSSOT.md` | 実装前チェック項目21件、実装タスク5フェーズ、テスト計画（**v2.0正本**） |
...
│   ├── 11_会話解釈SSOT.md         # v2.0 会話解釈設計
│   ├── 12_記録確認フローSSOT.md    # v2.0 記録確認フロー
│   ├── 13_パーソナルメモリSSOT.md  # v2.0 パーソナルメモリ
│   └── 14_技術設計チェックリストSSOT.md # v2.0 技術チェックリスト
├── migrations/                    # D1 マイグレーション SQL
│   ├── 0001_init_accounts.sql
│   ├── ...
│   ├── 0012_meal_food_match.sql
│   ├── 0013_pending_clarifications.sql  # v2.0
│   ├── 0014_correction_history.sql      # v2.0
│   └── 0015_user_memory_items.sql       # v2.0
├── src/
│   ├── index.ts                   # Hono アプリエントリーポイント
│   ├── types/
│   │   ├── bindings.ts            # Cloudflare Bindings 型定義
│   │   ├── db.ts                  # DB テーブル型定義
│   │   ├── intent.ts              # v2.0 Unified Intent Schema 型定義
│   │   └── index.ts               # 型エクスポート
│   ├── middleware/
...
│   │   │   ├── interpretation.ts  # v2.0 Phase A: interpretMessage()
│   │   │   ├── rag.ts             # RAG 検索・回答生成
│   │   │   └── embeddings.ts      # ベクトル生成
│   │   └── line/
│   │       ├── process-line-event.ts  # LINE イベント処理メイン
│   │       ├── intake-flow.ts     # 問診フロー
│   │       ├── image-confirm-handler.ts  # 画像確認ハンドラ
│   │       ├── reply.ts           # LINE API 返信ヘルパー
│   │       └── verify-signature.ts  # Webhook 署名検証
│   ├── repositories/
│   │   ├── daily-logs-repo.ts
...
│   │   ├── pending-clarifications-repo.ts  # v2.0
│   │   ├── correction-history-repo.ts      # v2.0
│   │   └── user-memory-repo.ts             # v2.0
│   ├── jobs/
│   │   ├── daily-reminder.ts
│   │   ├── weekly-report.ts
│   │   └── image-analysis.ts
...
- **Status**: ✅ MVP v1.2 稼働中 / v2.0 設計・基盤実装完了
...
| 会話解釈設計 | `docs/11_会話解釈SSOT.md` | Phase A/B/C アーキテクチャ、Intent Schema、日付/食事区分解決ルール、保存確認ルール、修正ターゲット |
| 記録確認フロー | `docs/12_記録確認フローSSOT.md` | 明確化フロー、永続化レイヤー、修正フロー、保存判定フロー |
| パーソナルメモリ | `docs/13_パーソナルメモリSSOT.md` | ユーザー嗜好・習慣・アレルギーの蓄積・活用 |
| 技術設計チェックリスト | `docs/14_技術設計チェックリストSSOT.md` | 実装前チェック、タスク優先順位、テスト計画 |
