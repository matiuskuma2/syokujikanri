...
## 将来拡張（スコープ外）
- 高精度食事画像 API（日本食品データベース連携等、Phase 2 以降）
- デバイス連携（体重計・歩数計）
- 自動課金・OEM 多店舗
...
> **食事解析の方針**: Phase 1 は OpenAI Vision のみで実装。  
> Phase 2 以降で日本食品辞書（FoodData Central、日本食品標準成分表等）との連携を検討する。
...
| `docs/DATABASE.md` | DB 設計・テーブル定義・マイグレーション管理 |
| `docs/REPOSITORY.md` | **★ NEW** Repository 層 SQL 実装仕様（D1 prepared statements） |
| `docs/API.md` | API エンドポイント定義（リクエスト/レスポンス形式） |
| `docs/PROMPTS.md` | **★ UPDATED** OpenAI プロンプト設計（`src/services/ai/prompts.ts` 実装仕様） |
| `docs/BOT_FLOW.md` | BOT フロー・ステップコード・キーワードトリガー定義 |
| `docs/DEPLOYMENT.md` | デプロイ手順（ローカル・本番） |
| `docs/PROJECT_PLAN.md` | **★ NEW** フェーズ別実装計画・次ステップ |
