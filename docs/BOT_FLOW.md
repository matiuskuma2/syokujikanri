...
画像受信
  │
  ├── OpenAI Vision で分類
  │   └── 判定: meal_photo / nutrition_label / body_scale / food_package / progress_body_photo / other / unknown
  │
  ├── meal_photo
  │   ├── 料理名候補抽出（MEAL_IMAGE_ESTIMATION_PROMPT）
  │   ├── 分量推定
  │   ├── PFC 推定
  │   └── 確認メッセージ送信 → record_waiting_meal_confirm
  │
  ├── nutrition_label
  │   ├── 栄養成分表テキスト抽出（NUTRITION_LABEL_PROMPT）
  │   └── meal_entries に保存
  │
  ├── body_scale
  │   ├── 体重数値抽出（BODY_SCALE_PROMPT）
  │   └── body_metrics に保存
  │
  ├── food_package  ← nutrition_label と同一フローで処理
  │   ├── 栄養成分表テキスト抽出（NUTRITION_LABEL_PROMPT）
  │   └── meal_entries に保存
  │
  ├── progress_body_photo
  ...
  │
  ├── other
  │   └── image_intake_results に記録のみ（ユーザー通知なし）
  │
  └── unknown
